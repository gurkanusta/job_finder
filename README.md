...
| LinkedIn | `linkedin` (email) / `linkedinjobs` (API) | Gmail'e gelen **Job Alert** mailini IMAP ile okuma **ve** LinkedIn'in public guest jobs API'sinden veri çekme | İki yöntem de scrape edilmez |
...
2. Bot adı ve kullanıcı adı ver. Sonunda sana bir **token** verir:
   `123456789:***` → bu **`TELEGRAM_BOT_TOKEN`**.
...
```jsonc
{
  ...
  \"linkedinJobs\": {
    \"keywords\": [\"junior backend\", \"junior developer\", \"junior software\", \"junior java\", \"junior .net\", \"junior python\", \"junior full stack\", \"new grad software\", \"yeni mezun yazılım\", \"associate software engineer\"],
...
- LinkedIn doğrudan scrape edilmez; sadece kendi gelen kutundaki alert maili okunur **veya** LinkedIn'in resmi guest jobs API'sinden veri çekilir.
